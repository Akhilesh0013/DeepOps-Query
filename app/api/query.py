...
from app.security.content_moderation import moderate_and_redact, moderate_input
...
async def query(
    ...
    # Layer 4 (content moderation in): toxicity check + regex PII only (no NER on input)
    mod_allowed, moderated_in, mod_reason = moderate_input(restructured)
    if not mod_allowed:
