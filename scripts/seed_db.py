...
def _ingest_one(processor, src: Path, idx: int, total: int, counters: dict,
    ...
    try:
        ...
        # Bulk ingestion shouldn't populate the embedding cache: chunk vectors live in
        # Qdrant and won't be re-queried by exact text, and caching them all exhausts
        # the Redis quota.
        embeddings = embed_texts_fn(texts, use_cache=False)
