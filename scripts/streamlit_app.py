...
def _render_auth_error(status: int, payload: Any, *, action: str, username: str = "") -> bool:
    """Show user-friendly auth errors. Returns True if a message was shown."""
    detail = payload.get("detail", "") if isinstance(payload, dict) else ""

    if action == "register" and status == 409:
        name = username or "This username"
        st.warning(
            f"⚠️ **{name}** already has an account. "
            "Switch to the **Login** tab and sign in with that username and password.",
            icon="👤",
        )
        return True
    ...
    if status == 429:
        if action == "register":
            st.warning(
                "⏳ Too many registration attempts (limit: 3 per hour). "
                "Wait a while and try again, or use the **Login** tab if you already registered.",
                icon="🚦",
            )
        else:
            st.warning(
    ...
    if action == "login" and status == 401:
        st.error("❌ Invalid username or password. Check your credentials and try again.")
        return True
    ...
    with tab_reg:
        ...
        if st.button("Create Account", width="stretch", key="btn_register"):
            ...
            if status in (200, 201) and isinstance(payload, dict) and "token" in payload:
                ...
                st.rerun()
            elif not _render_auth_error(status, payload, action="register", username=reg_user):
    ...
    with tab_log:
        ...
        if st.button("Login", width="stretch", key="btn_login"):
            ...
            if status == 200 and isinstance(payload, dict) and "token" in payload:
                ...
                st.rerun()
            elif not _render_auth_error(status, payload, action="login", username=log_user):
                _render_response_card(status, payload)
...
def _query_section(base_url: str, lesson_info: dict) -> None:
    ...
    # Query input. The widget value is managed via session state (key="q_question"),
    # so seed the default there instead of passing `value=` (which Streamlit warns about
    # when the key is also written by the preset buttons).
    if "q_question" not in st.session_state:
        st.session_state["q_question"] = "How do containers share resources within a Pod?"
